package com.begodya.bbsdkframework.security;
 ...
 */
public class SDKHttpClientUtil {

    private static String TAG = "SDKHttpClientUtil";
...
//            HttpURLConnection localConnection = (HttpURLConnection) localURL.openConnection();
            ...
            if (localConnection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                return SDKStreamUtil.readStreamToString(is, ENCODING);
            }
    ...
    public static String post(String url, Map<String, String> params) {
        ...
        try {
            ...
            if (localConnection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                // 取得该连接的输入流，以读取响应内容
                is = localConnection.getInputStream();
                String message = SDKStreamUtil.readStreamToString(is, ENCODING);
                // 读取服务器的响应内容并显示
                return message;
            } else {
                InputStream errorStream = localConnection.getErrorStream();
                String message = SDKStreamUtil.readStreamToString(errorStream, ENCODING);
                return message;
            }
        } catch (Exception e) {
            e.printStackTrace();
